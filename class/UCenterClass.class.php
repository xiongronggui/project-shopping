<?php
	...
	class UCenterClass extends BaseClass
	{
		...
		function goodsOrderListByWhere($where)
		{
			...
			if ($where)
			{
				$sql = "select goods_order.*, core_user.mobile from goods_order inner join core_user on goods_order.uid = core_user.id where   order_status = 1 and $where"; 
				$result = $db->query($sql) or die($db->error());
		...
		*/
		...
		/*兑换充值验证*/
		function goodsOrderChange($order_id,$code,$recharge_platform,$recharge_account,$usetime)
		{
			global $db;
			
			if ($order_id)
			{
				$sql =  "select *,mobile from goods_order inner join core_user on goods_order.uid = core_user.id where order_id = '$order_id' and order_status = 1 and usetime = 0"; 
				$result = $db->query($sql) or die($db->error());
				$rs = $db->fetch_array($result);
				if ($rs['order_id'])
				{
					$mobile =  $rs['mobile'];
					$mobile_code = "select code from core_user_sms where mobile = $mobile and type_id = 3 and code = $code";
					$code_result = $db->query($mobile_code) or die($db->error());
					$code_rs = $db->fetch_array($code_result);
					if($code_rs){
						$total = $rs['total'];
						$res['IsSuccess'] = true;
						$res['Message'] = '';
						$rs['recharge_platform'] = $recharge_platform;
						$rs['recharge_account'] = $recharge_account;
						$rs['recharge_total'] = $total;
						$rs['usetime'] = $usetime;
						$rs['pwd'] = authcode($rs['pwd']);
						$res['data'] = $rs;
						/*$update_sql = "update goods_order set recharge_platform = '$recharge_platform',recharge_account=$recharge_account,recharge_total =$total,usetime = $usetime where order_id = $order_id";
						$change_result = $db->query($update_sql) or die($db->error());
						if($change_result){
							$res['IsSuccess'] = true;
							$res['Message'] = '';
							$rs['recharge_platform'] = $recharge_platform;
		...
		/*兑换充值更改状态*/
		function goodsOrderChangeStatus($orderinfo)
		{
			global $db;
			if ($orderinfo['order_id'])
			{
				$order_id = $orderinfo['order_id'];
				$recharge_platform = $orderinfo['recharge_platform'];
				$recharge_account = $orderinfo['recharge_account'];
				$recharge_total = $orderinfo['recharge_total'];
				$usetime = $orderinfo['usetime'];
				$notify_status = 2;
				$total = $orderinfo['total'];
				$update_sql = "update goods_order set recharge_platform = '$recharge_platform',recharge_account=$recharge_account,recharge_total =$total,usetime = $usetime,notify_status = $notify_status where order_id = '$order_id'";
				$change_result = $db->query($update_sql) or die($db->error());
				if($change_result){
					$res['IsSuccess'] = true;
					$res['Message'] = '';
		...
		/*根据条件搜索订单*/
		function goodsOrderMobileByOrderID($order_id)
		{
			global $db;
			
			if ($order_id)
			{
				$sql = "select order_id,core_user.mobile from goods_order inner join core_user on goods_order.uid = core_user.id where   order_id = $order_id"; 
				$result = $db->query($sql) or die($db->error());
				$rs = $db->fetch_array($result);
				if ($rs)
				{
