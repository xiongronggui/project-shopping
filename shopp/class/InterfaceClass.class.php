<?php
	...
	class InterfaceClass extends BaseClass
	{
		...
		//充值完成状态修改
		function changeCodeStatus($order_id, $password, $code, $recharge_platform, $recharge_account = '', $recharge_total ='')
		{
			global $db;
			logInfo("changeCodeStatus->订单号:".$order_id);
			if ($order_id && $password && $code && $recharge_platform)
			{
				$pwd = authcode($password, "ENCODE");
