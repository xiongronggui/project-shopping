<?php
	...
	if($payResult)
	{
		...
		$sql = "select goods_order.*, core_user.mobile from goods_order inner join core_user on goods_order.uid = core_user.id where order_status = 0 and order_id = '$orderNo'";
		...
		if ($resultu)
		{
			...
			logInfo('push->U+:start');
			require_once 'class/FaceClass.class.php';
            $user = new FaceClass();
            $user_data = $user->golden($rs['uid'],$rs['paytime']);
			logInfo('push->U+:'.json_encode($user_data));
            if($user_data['success']){
                $url = 'http://139.199.75.178:8080/achieveBuyCount';
               $reresult = _curlPost($url,$user_data['data']);
			   logInfo('push-return->U+:'.json_encode($reresult));
            }
