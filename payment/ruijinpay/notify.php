...
class ruijin_notify
{
	...
	public function __construct($config)
	{
		...
		logInfo('ruijin:'.json_encode($data));
		$this->param = $data;
	...
	public function submit()
	{
		...
		if(!$pay->isValid($valid_param, $sign)){
			logInfo('ruijin:error3');
			exit('error3');
		}
		$payment_status = $this->param['paySt'];
		if($payment_status != 2){
			logInfo('ruijin:error4');
			exit('error4');
		}
		return true;
