<?php
	...
	class GoodsClass extends BaseClass
	{
		...
		function goodsOrderList($page, $keys='', $order_status, $use_status, $price_start='', $price_end='', $paytime_start='', $paytime_end='', $usetime_start='', $usetime_end='')
		{
			...
			if ($keys)
			{
				$conn .= " and (order_id like '%$keys%' or core_user.mobile like '%$keys%' or core_user.realname like '%$keys%' or channel like '%$keys%')";
			}
			...
			$sqlc = "select goods_order.*, core_user.mobile, core_user.realname,core_service_charge.scharge from goods_order inner join core_user on goods_order.uid = core_user.id left join core_service_charge on SUBSTRING_INDEX(goods_order.channel,'-',1) = core_service_charge.`name`  where 1 = 1" . $conn . " order by goods_order.addtime desc limit ". $offest .",".$each_disNums;
			...
			while($rs = $db->fetch_array($resultc))
			{
				...
				$rs['scharge'] = number_format($rs['scharge']*100,2);
				$rs['service_charge'] = number_format($rs['service_charge']/100,2);
		...
		function goodsOrderListExcel($keys='', $order_status, $use_status, $price_start='', $price_end='', $paytime_start='', $paytime_end='', $usetime_start='', $usetime_end='')
		{
			...
			if ($keys)
			{
				$conn .= " and (order_id like '%$keys%' or mobile like '%$keys%' or realname like '%$keys%' or channel like '%$keys%')";
			}
			...
			$sql = "select goods_order.*, core_user.mobile, core_user.realname,core_service_charge.scharge from goods_order inner join core_user on goods_order.uid = core_user.id left join core_service_charge on SUBSTRING_INDEX(goods_order.channel,'-',1) = core_service_charge.`name` where 1 = 1" . $conn . " order by goods_order.order_id desc";
			$result = $db->query($sql) or die($db->error());
			while($rs = $db->fetch_array($result))
			{
				$rs['price_show'] = "$" . number_format($rs['price']/100, 2, ".", "");
				...
				$rss['service_charge'] = "￥" . number_format($rs['service_charge']/100,2);
		...
		function goodsOrderRechargeList($page, $keys, $recharge_platform_id, $price_start, $price_end, $paytime_start, $paytime_end, $usetime_start, $usetime_end)
		{
			...
			if ($keys)
			{
				$conn .= " and (order_id like '%$keys%' or mobile like '%$keys%' or realname like '%$keys%' or recharge_account like '%$keys%' or channel like '%$keys%')";
			}
			...
			$sub_page = new SubPages($each_disNums,$nums,$current_page,$sub_pages,$subPage_link,$subPage_type);
			$page_info = $sub_page->show_SubPages(2);
			$sqlc = "select goods_order.*, core_user.mobile, core_user.realname,core_service_charge.scharge from goods_order inner join core_user on goods_order.uid = core_user.id left join core_service_charge on SUBSTRING_INDEX(goods_order.channel,'-',1) = core_service_charge.`name`  where usetime <> 0" . $conn . " order by goods_order.usetime desc limit ". $offest .",".$each_disNums;
			$resultc = $db->query($sqlc) or die($db->error());
			while($rs = $db->fetch_array($resultc))
			{
				$rs['price_show'] = "$ " . number_format($rs['price']/100, 2);
		...
		function goodsOrderRechargeListExcel($keys, $recharge_platform_id, $price_start, $price_end, $paytime_start, $paytime_end, $usetime_start, $usetime_end)
		{
			...
			$sql = "select goods_order.*, core_user.mobile, core_user.realname,core_service_charge.scharge from goods_order inner join core_user on goods_order.uid = core_user.id left join core_service_charge on SUBSTRING_INDEX(goods_order.channel,'-',1) = core_service_charge.`name`  where usetime <> 0" . $conn . " order by goods_order.usetime desc";
			$result = $db->query($sql) or die($db->error());
			while($rs = $db->fetch_array($result))
			{
				$rs['price_show'] = "$ " . number_format($rs['price']/100, 2, ".", "");
				...
				$rss['total_show'] = $rs['total_show'];
				$rss['use_rate'] = $rs['use_rate'];
				$rss['service_charge'] ="￥" . number_format($rs['service_charge'],2);
				$rss['channel'] = $rs['channel'];
				$rss['paytime_show'] = $rs['paytime_show'];
		...
		//转账/汇款,人工确认支付
		function order_edit($order_id,$rmbmoney,$money)
        {
            global $db;
            $paytime = time();
        ...
        //支付回调失败,人工处理.
        function order_status_edit($order_id)
        {
        	global $db;
        	$sql = "select order_id,uid,mobile from goods_order inner join core_user on goods_order.uid = core_user.id where order_id = '$order_id'";
			$result = $db->query($sql) or die($db->error());
			$nums = (mysql_num_rows($result));
			if($nums){
				$rs = $db->fetch_array($result);
	            ...
	            if($resultu){
					...
					$req->setSmsParam("{'time':'$paytime', 'order_id':'$order_id', 'pwd':'$password'}");
					$req->setRecNum($mobile);
					$req->setSmsTemplateCode($temp_code);
					$resp = $c->execute($req);
					/*发送短信end*/
	                $this->goToUrl("订单状态更新成功", "order_status_change.php");
	            }else{
	                //return array('success'=>0,'message'=>'修改失败');
	                $this->goToUrl("订单状态更新失败", "order_status_change.php");
	            }
			}else{
				$this->goToUrl("更新的订单号不存在", "order_status_change.php");
			}
        }
	}
?>